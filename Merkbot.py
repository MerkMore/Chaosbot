...
class Chaosbot(sc2.BotAI):
    ...
    do_slowdown = False
    slowdown_frames = 0
    slowness = 0.25 # realtime is around 0.05
    ...
    enemy_structureinfo_of_expo = {} # per expo: set of structureinfo = tpa = (typ,pos,tag) last seen
    # strength = dps * health
    ...
    fearframe_of_banshee = {} # per bansheetag a number
    ...
    # for workerrush:
    enemyworkers = set() # of tag of reachable enemy worker
    hisleftmim = None
    hisrightmim = None
    hisrampmim = None
    hismidmim = None
    ...
    followbound = 3.0 # follow that enemy
    ...
    #
    #   ############### SCV MANAGEMENT
    #   traveller -> settler -> fencer -> builder -> gasper scvs have a goal
    ...
    good_jobs = ('builder','settler','fencer','traveller','repairer','escorter','scout','cheeser')
    bad_jobs = ('gasminer','mimminer','applicant','defender','gasper')
    no_jobs = ('idler','suicider','volunteer','inshock','fleeer')
    #
    jobs_may_idle = ('mimminer','fencer','settler','scout','cheeser','applicant','idler','gasper')
    #
    expected_orders = set()
    #
    job_of_scvt = {}
    ...
    mimpos_of_minert = {}
    phase_of_minert = {}
        # A near hall, starting gathering trip
        # W halting to prevent scv clashes
        # G gathering trip
        # H moving from near to patch to the patch
        # I gathering at the patch
        # R returnminerals trip
        # S moving from near the hall to the hall
        # T returnminerals at the hall
    count_of_minert = {} # frames since phase R ended
    lastdist_of_minert = {}
    basepos_of_minert = {}
    speedbases = 0
    minclick_of_mimt = {}
    ...
    #
    throwspots = [] # (thing spot status='dream'/'thought'/'prep' owner prio=1/2/3)     (thing,spot) will be unique
    #
    ...
    #
    # to throw another thing, check if you still are rich
    rich_hash = 0
    rich_minerals = 450
    rich_vespene = 350
    #
    ...
    production_pause_egg = set() # of (thing, amount, building). Postpone adlib thing production at amount, until a building is in eggs or birds.
    # to abort plans
    buildorderdelay = 0
    crad_places = set()
    bui_min_lab = {}
    ...
    async def my_init(self):
        ...
        self.init_upgrade(TERRANBUILDINGARMOR, 100, COMMANDCENTER)
        ...
        # add some constraints that do not exist, to encourage parallel upgrading later:
        self.init_techtree(TERRANVEHICLEANDSHIPARMORSLEVEL2,TERRANSHIPWEAPONSLEVEL1)
        self.init_techtree(TERRANVEHICLEANDSHIPARMORSLEVEL3,TERRANSHIPWEAPONSLEVEL2)
        self.init_techtree(TERRANSHIPWEAPONSLEVEL2,TERRANVEHICLEANDSHIPARMORSLEVEL1)
        self.init_techtree(TERRANSHIPWEAPONSLEVEL3,TERRANVEHICLEANDSHIPARMORSLEVEL2)
        #
        ...
        #
        if self.enemy_race != Race.Zerg:
            self.enemy_structureinfo.add((HATCHERY,self.enemy_pos,self.notag))
        elif self.enemy_race != Race.Protoss:
            self.enemy_structureinfo.add((NEXUS, self.enemy_pos, self.notag))
        else: # random or terran    
            self.enemy_structureinfo.add((COMMANDCENTER, self.enemy_pos, self.notag))
        # vulture
        self.init_vulture()
        ...
        # INFESTEDCOCOON is often not in placement.txt, init it here
        ...
        if self.game_choice[29]:
            ...
            while facs < 3:
                ...
                for mim in self.mineral_field:
                    if self.near(place,mim.position,3):
                        nearminerals = True
                if not nearminerals:
        ...
        if self.game_choice[32]:
            ...
            around = Point2((x,y))
            barpos = self.somewhere_around(around)
            ...
            around = Point2((x,y))
            facpos = self.somewhere_around(around)
        ...
        if self.game_choice[33]:
            ...
            self.buildseries_opening = [SUPPLYDEPOT, BARRACKS, REFINERY, SUPPLYDEPOT, MARINE, REFINERY, FACTORY, MARINE,
                                       STARPORT, COMMANDCENTER, FUSIONCORE, STARPORTTECHLAB, BATTLECRUISER]
        if self.game_choice[34]:
            self.opening_name = 'workerrush-bunker'
            self.rushopening = True
            ...
            if (self.enemy_race == Race.Zerg):
                bunker_raw_pos = self.enemyramp_pos.towards(self.enemynatural_pos,-3)
            else:
        ...
        for scv in self.units(SCV):
            if scv.tag not in self.speciality_of_tag:
                self.job_of_scvt[scv.tag] = 'mimminer'
                amimt = self.notag
                best_sd = 99999
                for mim in self.mineral_field:
                    if self.count_of_mimt[mim.tag] < 1:
        ...
        pl.close()
        # chat
        await self._client.chat_send('Chaosbot version 5 may 2021, made by MerkMore', team_only=False)
        #
        #layout_if.photo_layout()

    def somewhere_around(self, around) -> Point2:
        ok = False
        while not ok:
            ok = True
            place = self.random_mappoint()
            if (not self.near(around, place, 30)):
                ok = False
            place = self.place_around(BARRACKS, place)
            for mim in self.mineral_field:
                if self.near(place, mim.position, 4):
                    ok = False
        return place
    ...
    def init_marine_opening(self, nbases):
        ...
        self.opening_create_units = nbases * 24 # 4 barracks * 6 marines/barracks
    ...
    def log_throwspots(self):
        if self.do_log_throwspots:
            if len(self.throwspots) > 0:
                ...
                print(' --- Total gas '+str(self.rich_vespene) + ' minerals ' + str(self.rich_minerals))
                print(' In purse '+str(self.purse.vespene) + ' minerals ' + str(self.purse.minerals))
                print(' I Have gas '+ str(self.vespene) + ' minerals ' + str(self.minerals))
                    ...
                    if job == 'defender':
                        if scv.orders[0].ability.id == AbilityId.ATTACK:
                            if scv.orders[1].ability.id == AbilityId.ATTACK:
                                normal = True
                    if job == 'mimminer':
                        if scv.orders[0].ability.id == AbilityId.HARVEST_RETURN:
                            if scv.orders[1].ability.id == AbilityId.HARVEST_RETURN:
                                normal = True
                if (len(scv.orders) == 1):
    ...
    def maxam_of_thing(self,thing) -> int:
        ...
        if thing in self.all_structures_tobuildwalk:
            ...
            if thing == BARRACKS:
                if self.opening_name.find('marine') < 0: # not
                    maxam = len(self.all_bases)*2+1
            if thing == STARPORT:
                if self.opening_name not in {'two liberators','banshees'}:
    ...
    def init_step_expo(self):
        ...
        for tpa in self.enemy_structureinfo:
            ...
            self.enemy_structureinfo_of_expo[expo].add(tpa)
    ...
    def get_total_cost(self,building) -> int:
        if building == NUKESILONOVA:
            return Cost(100,100)
        else:
            return self.calculate_unit_value(building)

    def get_added_cost(self,building) -> int:
        if building == NUKESILONOVA:
            return Cost(100,100)
        else:
    ...
    def calculate_rich(self):
        # reserve money for buildorder_exe, throwspots and opening_create_units
        hash = len(self.buildorder_exe) + self.opening_create_units
        for (thing,pos,status,owner,prio) in self.throwspots:
            hash += pos.x
        if hash != self.rich_hash: # calc reservation
            mim = 450 # bc + bit
            gas = 350 # bc + bit
            ...
            for (item, goal, status) in self.buildorder_exe:
                ...
                future *= 0.667
            # protect throwspots:
            # use future system although there is no order
            future = 1.0
            for tps in self.throwspots:
                ...
                gas += future * cost.vespene
                future *= 0.667
            # protect opening_create_units
            # these build quick, use full protection
            if self.opening_create_units > 0:
                ...
                for tps in self.throwspots:
                    ...
                    if th == self.opening_create_kind:
                        mar_todo -= 1
                mar_todo = max(0,mar_todo)
                mar_cost = self.get_added_cost(self.opening_create_kind)
                mim += mar_cost.minerals * mar_todo
                gas += mar_cost.vespene * mar_todo
            #
            self.rich_minerals = mim
            self.rich_vespene = gas
            self.rich_hash = hash

    def rich(self) -> bool:
        self.calculate_rich()
        return (self.vespene-self.purse.vespene >= self.rich_vespene) and (self.minerals-self.purse.minerals >= self.rich_minerals)

    def rich_nogas(self) -> bool:
        # do i have minerals to throw a new thing?
        self.calculate_rich()
        return (self.minerals - self.purse.minerals >= self.rich_minerals)
        ...
        for mim in self.minerals_of_expo[expo]:
            ...
            if mimt in self.all_mimt:
                if self.count_of_mimt[mimt] < 2:
                    sd = self.sdist(mim.position,point) + 30 * self.count_of_mimt[mimt]
                    if sd < best_sdist:
                        best_sdist = sd
                        best_mimt = mimt
        if best_sdist == 99999:
    ...
    def bagofcradle_of_bagofthings(self):
        ...
        # Accept buildings beyond the bag, i.e. existing or being built
        stri = 'bagofthings: '
            ...
            for th in bag:
                for pair in self.techtree:
                    if pair[0] == th:
                        ...
                        if otherth in self.all_structures:
                            haveit = haveit or (len(self.structures(otherth)) > 0)
                        else:
                            haveit = haveit or (len(self.units(otherth)) > 0)
                        if not haveit:
                            extra.add(otherth)
                            stable = False
            for th in extra:
    ...
    def buildseries_of_bagoftree(self):
        ...
        series = []
        oldidea = [COMMANDCENTER] # initial structure
        for thing in self.buildseries_opening:
            oldidea.append(self.basekind_of(thing))
        for (thing, pos) in self.birds:
            if thing in oldidea:
                del oldidea[oldidea.index(thing)]
            # hidden structure
            if thing in {ORBITALCOMMAND,PLANETARYFORTRESS}:
                if COMMANDCENTER in oldidea:
                    del oldidea[oldidea.index(COMMANDCENTER)]
...
#*********************************************************************************************************************
    ...
    async def gamestate(self):
            ...
            if (bc.tag in self.emotion_of_unittag):
                if (self.emotion_of_unittag[bc.tag] == 'bcrecovering'):
                    if (bc.health<self.maxhealth[bc.type_id]):
                        new_emotion_of_unittag[bc.tag] = 'bcrecovering'
                    else:
                ...
                new_emotion_of_unittag[bc.tag] = 'lazy'
        # also siegetanks etc
        for tnk in self.units:
            if tnk.type_id != BATTLECRUISER:
        ...
        #
        #   gym_of_strt contains tags of living commandcenters, barracks etc. May be not ready.
        new_gym_of_strt = {}
        for cc in self.structures:
        ...
        # add half of future army
        for (marty,bartype,pos,dura) in self.eggs:
            if (marty in self.all_army):
                self.army_supply += self.supply_of_army[marty] / 2
        for (marty,bartype,pos,dura,owner) in self.preps:
            if (marty in self.all_army):
                self.army_supply += self.supply_of_army[marty] / 2
        for (thing, pos, status, owner, priority) in self.throwspots:
            if (thing in self.all_army):
                if status in {'dream','thought'}:
                    self.army_supply += self.supply_of_army[thing] / 2
        #
        # enemy_structureinfo
        itchanged = False
        for ene in self.enemy_structures:
            ...
            if goodtpa not in self.enemy_structureinfo:
                ...
                for tpa in self.enemy_structureinfo:
                    (typ,pos,tag) = tpa
                    if self.near(pos,ene.position,1.0) or (tag == ene.tag):
                        todel.add(tpa)
                self.enemy_structureinfo -= todel
                # add new info
                self.enemy_structureinfo.add(goodtpa)
        if itchanged:
            self.init_undetect()
        # some average counts related to supply from some gsl games:
    ...
    async def build_minima(self):
        ...
        if (self.opening_create_units <= 0) or (self.minerals > 999):
            ...
            if self.rich_nogas():
                if (ccs == 0):
                    self.throw_somewhere(COMMANDCENTER,'build_minima',1)
            if self.rich_nogas():
                if (not self.we_started_a(SUPPLYDEPOT)):
                    self.throw_somewhere(SUPPLYDEPOT,'build_minima',1)
            if self.rich_nogas():
                if (not self.we_started_a(BARRACKS)):
                    self.throw_somewhere(BARRACKS,'build_minima',1)
            if self.rich_nogas():
                if (not self.we_started_a(REFINERY)):
                    self.throw_somewhere(REFINERY,'build_minima',1)
            if self.rich_nogas():
                if (not self.we_started_a(ENGINEERINGBAY)) and (ccs >= 2):
                    self.throw_somewhere(ENGINEERINGBAY,'build_minima',1)
            if self.rich_nogas():
                if (not self.we_started_a(FACTORY)) and (ccs >= 2):
                    self.throw_somewhere(FACTORY,'build_minima',1)
            if self.rich_nogas():
                if (not self.we_started_a(MISSILETURRET)) and (ccs >= 2):
                    self.throw_somewhere(MISSILETURRET, 'build_minima', 1)
            if self.rich():
                if (not self.we_started_a(STARPORT)) and (ccs >= 2):
                    self.throw_somewhere(STARPORT,'build_minima',1)
            if self.rich():
                if (not self.we_started_a(FUSIONCORE)) and (ccs >= 2):
                    self.throw_somewhere(FUSIONCORE,'build_minima',1)
            if self.rich():
                if (not self.we_started_a(ARMORY)) and (ccs >= 4):
                    self.throw_somewhere(ARMORY,'build_minima',1)
            if self.rich():
                if (not self.we_started_a(RAVEN)) and (bcs >= 4):
                    self.throw_somewhere(RAVEN,'build_minima',1)
            if self.rich():
                if (not self.we_started_a(FACTORYTECHLAB)):
                    self.throw_somewhere(FACTORYTECHLAB,'build_minima',1)
            if self.rich():
                if (not self.we_started_a(STARPORTTECHLAB)):
                    self.throw_somewhere(STARPORTTECHLAB,'build_minima',1)
    ...
    def remove_from_planning_etc(self, thing, place):
        ...
        # from buidseries_exe and the bags
    ...
    async def implementing_buildorder_exe(self):
        ...
        for nr in range(0,len(self.buildorder_exe)):
            ...
            if status == 'dream':
                ...
                # delay building during workerrush
                        ...
                        if thing in self.all_structures_tobuildwalk:
                            if sr <= 0:
                                if scvt not in self.goal_of_trabu_scvt:
                                    if await self.build_thing_tobuildwalk(scvt,thing, place, owner):
                                ...
                                for abar in self.structures:
                                    if self.position_of_building(abar) == place:
                                        if abar.tag in (self.ambition_of_strt.keys() | self.gym_of_strt.keys()):
                                            inuse = True
                                if not inuse:
                                    if await self.build_thing(thing, place, owner):
                                        del self.thoughts[self.thoughts.index((thing, pl, owner))]
                                        self.buildorder_exe[nr] = (th, place, 'prep')
                        else: # upgr, army
                            if sr <= 0:
    ...
    def throw_at_spot(self, th, po, ow, pri):
    ...
    def add_thought(self, thing,pos,owner) -> bool:
        ...
        if bad:
            # remove this from new throwspots and chosenplaces
            todel = []
            for tps in self.throwspots:
                (th, po, status, ow, pri) = tps
    ...
    def throw_treepayno(self, thing, owner, priority):
        if not self.we_started_a(thing):
            seenthrown = False
            for (th, pos, status, ow, prio) in self.throwspots:
                if (th == thing):
                    seenthrown = True
            if not seenthrown:
            ...
            if thing in self.all_structures_tobuildwalk:
                ...
                if len(sps) > 0:
                    ...
                    if (thing, place) not in self.unthinkable:
                        foundplace = True
            if foundplace:
                self.throw_at_spot(thing,place,owner,priority)
    ...
    async def throw_advance(self):
        ...
        for priority in [1, 2, 3]:
            ...
            while nr < len(self.throwspots): # a throwspot can be deleted in the loop
                ...
                (thing, place, status, ow, pri) = tps
                if (status == 'dream') and (pri == priority):
                    if self.add_thought(thing, place, ow):
                        ...
                        self.thoughts.append((thing, place, ow))
                        self.throwspots.append((thing, place, 'thought', ow, pri))
                nr += 1
                ...
                if (status == 'thought') and (pri == priority):
                    if thing in self.all_structures_tobuildwalk:
                        ...
                        if await self.build_thing_tobuildwalk(scvt, thing, place, ow):
                            ...
                            self.throwspots.append((thing, place, 'prep', ow, pri))
    ...
    def opening_create_cut(self):
        ...
        if self.opening_create_units > 0:
            ...
            if self.opening_create_units + have < self.opening_create_hadmax:
                            ...
                            if upg != NUKESILONOVA:
                                if self.check_techtree(upg):
                                    if not self.we_started_a(upg):
                                        if todo > 0:
                                            todo -= 1
                                            self.throw_somewhere(upg,'upgrades_adlib',3)
        if self.rich():
            ...
            if loved >= 6:
                ...
                if not self.we_started_a(FACTORYTECHLAB):
                    self.throw_somewhere(FACTORYTECHLAB,'upgrades_adlib',3)
        if self.rich():
            # second armory
            bcs = len(self.units(BATTLECRUISER))
            if (bcs >= 4) and (self.we_started_amount(ARMORY) < 2):
                self.throw_somewhere(ARMORY, 'upgrades_adlib', 2)


    def armybuildings_adlib(self):
        # reactive adding barracks, factory or starport with lab
        if self.rich():
            ccs = self.we_started_amount(COMMANDCENTER) + self.structures(PLANETARYFORTRESS).amount \
                  + self.structures(ORBITALCOMMAND).amount
            if ccs >= 3:
                    ...
                    if lab_type in self.all_labs:
                        if self.rich():
                            ...
                            if not has_idle:
                                ...
                                for (th, po, status, ow, pri) in self.throwspots:
                                    if (th == lab_type) and (status == 'dream'):
                                        canlab -=1
                                    if (th == bui_type) and (status == 'dream'):
                                        canlab +=1
                                if canlab > 0:
                                    self.throw_somewhere(lab_type,'armybuildings_adlib',2)
                                else:
                                    bui_started = False
                                    for (th, po, status, ow, pri) in self.throwspots:
                                        if th == bui_type:
    ...
    def army_adlib(self):
        ...
        if self.rich():
            ...
            for ene in self.enemy_units:
                if (ene.type_id in self.viking_targets) and (ene.type_id != OVERLORD):
                    vikings += 1.5
            if (self.army_supply < self.good_army_supply) or (self.minerals > 600):  # army
                # supply ghost+raven = 4; fill to 196
                if self.supply_used < 191:
                    if self.rich():
    ...
    def turrets_adlib(self):
        ...
        if self.rich_nogas():
            if self.we_finished_a(ENGINEERINGBAY):
                ...
                if (mts < ccs * 1) and (self.minerals > 450):
                    self.throw_somewhere(MISSILETURRET,'turrets_adlib',2)
                elif (mts < ccs * 4) and (self.minerals > 650):
                    self.throw_somewhere(MISSILETURRET,'turrets_adlib',2)
                elif (mts < ccs * 8) and (self.minerals > 1050):
                    self.throw_somewhere(MISSILETURRET,'turrets_adlib',2)
    ...
    def prepped_to_egg(self,  martype, bartype, pos):
        ...
        self.eggs.add((martype, bartype, pos, 20))
        # del preps
        todel = set()
                        ...
                        if scv.tag == scvt:
                            if self.job_of_scvt[scvt] == 'fencer':
                                ...
                                if is_building_it:
                                    ...
                                    todel.add(items)
                                    self.eggs.add((building, scvt, goal, 20))
                                    throwtodel = set()
                                    for tps in self.throwspots:
                                        (th, po, status, ow, pri) = tps
                                        if (th == building) and (po == goal):
                                            throwtodel.add(tps)
                                    for tps in throwtodel:
                                        del self.throwspots[self.throwspots.index(tps)]
                ...
                for scv in self.units(SCV):
                    if scv.tag == scvt:
                        ...
                        if self.we_started_amount(building) <= 1:
                            self.throw_somewhere(building,'do_buildandretry',1)
                        ...
                        if (len(scv.orders) == 0):
                            ...
                            for order in scv.orders:
                                if order.ability.id != AbilityId.MOVE:
                                    ...
                                    ok = False
                        if not ok:
                            self.log_success('Traveller went astray, rerouted')
                            scv.move(goal)
    ...
    def nearly_ready(self, cc,cctype, secs) -> bool:
        ok = (cc in self.structures(cctype).ready)
        if not ok:
            for (thing,scv,pos,dura) in self.eggs:
    ...
    def nearly_free(self, cc,cctype, secs) -> bool:
        ok = True
        for (martype, bartype, pos, dura) in self.eggs:
            if (bartype == cctype) and (pos == cc.position):
                ok = (dura <= secs)
        return ok
                                            ...
                                            if cc.tag not in self.gym_of_strt:
                                                if self.supply_check(trainee):
                                                    ...
                                                    if self.opening_create_kind == trainee:
                                                        self.log_success('Ha another ' + trainee.name + '!')
...
#   army movement routines
        ...
        if (not self.undetect_ok(goal)):
            ...
            while (not self.undetect_ok(goal)) and (tries < 7):
                ...
                for det in self.detect:
                    ...
                    elif self.near(det, goal, 11.5):
                        goal = det.towards(goal,11.5)
    ...
    def marine_fun(self):
        if self.opening_name.find('marine') >= 0:
            ...
            # 30 sec of new marines are sent in too
            # buildtime of a marine is 18 sec
            if self.frame % 17 == 16:
                if self.frame < self.marine_attackframe + 22.4 * 30:
                    for mar in self.units(MARINE):
                        if mar.tag not in self.speciality_of_tag:
                            mar.attack(self.marine_goal)
                            self.speciality_of_tag[mar.tag] = self.opening_name
    ...
    def lefttarget_righttarget(self):
        ...
        self.righttarget = self.enemy_pos
        rightsd = 99999
        for (typ, pos, tag) in self.enemy_structureinfo:
            if typ in self.hall_types:
    ...
    def sneakymid(self, comefrom,togoto) -> Point2:
        if self.near(comefrom, togoto, 5):
            ...
            mid = mid.towards(self.game_info.map_center, -dist / 5)
    ...
    async def attack_with_rest(self):
        ...
        left_building_target_tag = self.notag
        ...
        for tpa in self.enemy_structureinfo:
            ...
            if sd < bestsd:
                ...
                left_building_target_tag = tag
        right_building_target_tag = self.notag
        ...
        for cyc in self.units(CYCLONE):
            ...
            if self.no_doubling(cyct):
                if self.emotion_of_unittag[cyct] == 'resting':
                    ...
                    if side == 'L':
                        target_tag = left_building_target_tag
                        target_pos = left_building_target_pos
                    else:
                        target_tag = right_building_target_tag
                        target_pos = right_building_target_pos
                    if target_tag != self.notag:
    ...
    def use_cluster(self):
        for cluster in self.clusters:
            ...
            if cluster not in self.goal_of_cluster:
                points = set()
                for expopoint in self.expansion_locations_list:
                    point = expopoint.towards(self.game_info.map_center, -1)
                    if self.proxy(point):
                        expo = self.expo_of_pos(point)
                        owning = False
                        for cc in self.all_bases:
            ...
            if wmt not in self.speciality_of_tag:
                ...
                if self.emotion_of_unittag[wmt] == 'lodging':
                    if self.frame > self.mine_burried[wmt] + 350: # some 15 sec
                        if wm.is_idle: # prevent repeating command while unburrowing
    ...
    def eval_libspots(self):
        ...
        for spot in self.liberator_spots:
            ...
            thisspotlove = 0
            for tpa in self.enemy_structureinfo_of_expo[noseexpo]:
                (typ,pos,tag) = tpa
                if typ in self.hall_types:
                    thisspotlove += 100
                ...
                if (martype == LIBERATOR) and (dura < 8):
                    for scv in self.units(SCV):
                        if scv.tag == self.scout1_tag:
                            ...
                            self.throw_at_spot(BUNKER,self.cheese1_landing_pos,'liberator-tank',1)
        for lib in self.units(LIBERATORAG):
            if lib.tag in self.libspot:
                spot = self.libspot[lib.tag]
                oldname = spot[0]
            ...
            if tow.tag not in self.speciality_of_tag:
                ...
                for ene in self.enemy_units_of_expo[expo]:
                    if not ene.is_flying:
                        if (tow.position == self.loved_pos) or (self.near(ene.position,tow.position,10)):
                            enemies.add(ene.position)
                            enemy_power += self.unit_power(ene.type_id)
                            cx = cx + ene.position.x
                            cy = cy + ene.position.y
                            n_enemies += 1
                for tpa in self.enemy_structureinfo_of_expo[expo]:
                    (typ,pos,tag) = tpa
                    enemies.add(pos)
    ...
    async def bunkercheese(self):
        ...
        if self.cheese1_phase == 'Anobunker':
            if not self.we_started_at(BUNKER, self.cheese1_bunker_pos):
                self.throw_at_spot(BUNKER, self.cheese1_bunker_pos, 'bunkercheese', 1)
    ...
    async def cheese3_internals(self):
        ...
        if not self.we_started_at(BUNKER, self.cheese3_bunker_pos):
            if self.rich_nogas():
                self.throw_at_spot(BUNKER, self.cheese3_bunker_pos, 'cheese3_internals',1)
        if self.cheese3_bunker2_pos != self.nowhere:
            if not self.we_started_at(BUNKER, self.cheese3_bunker2_pos):
                if self.rich_nogas():
                    self.throw_at_spot(BUNKER, self.cheese3_bunker2_pos, 'cheese3_internals',1)
    ...
    def do_banshees(self):
        if self.opening_name.find('banshees') >= 0:
            # get starport and techlab
            for (martype, bartype, pos, dura) in self.eggs:
                if martype == BARRACKSTECHLAB:
                    self.toswap.add((BARRACKS,pos))
                    self.toswap.add((TECHLAB, pos))
                elif martype == FACTORYTECHLAB:
                    self.toswap.add((FACTORY, pos))
                    self.toswap.add((TECHLAB, pos))
                elif martype == STARPORT:
                    self.toswap.add((STARPORT, pos))
            # connect starport techlab
            combis = set()
            for (thing1,pos1) in self.toswap - self.swapped:
                if thing1 == STARPORT:
                    for (thing2, pos2) in self.toswap - self.swapped:
                        if thing2 == TECHLAB:
                            combis.add((pos1,pos2,self.sdist(pos1,pos2)))
            minsd = 9*9
            for (frompos,topos,sd) in combis:
                if sd < minsd:
                    combi = (frompos,topos)
                    minsd = sd
            if minsd < 9*9:
                ...
                (frompos,topos) = combi
                for (thing, pos) in self.toswap:
                    if (pos == frompos) or (pos == topos):
                        self.swapped.add((thing,pos))
            # lift barracks when techlab
            for (thingtype,pos) in self.birds:
                if thingtype in {FACTORY,BARRACKS}:
                    if (thingtype,pos) in self.swapped:
                        for bu in self.structures(thingtype):
                            if bu.position == pos:
                                # lift
                                goal = self.wall_barracks_pos
                                if thingtype == FACTORY:
                                ...
                                bu(AbilityId.LIFT)
                                # it becomes FACTORYFLYING, that prevents repeat
            # lift starport when ready
            for (thingtype,pos) in self.birds:
                if thingtype == STARPORT:
                    if (thingtype,pos) in self.swapped:
                        for bu in self.structures(thingtype):
                            if bu.position == pos:
                                # lift
            ...
            for bu in self.structures(STARPORTTECHLAB):
                if not self.we_started_a(upg):
                    place = self.position_of_building(bu)
                    self.throw_at_spot(upg, place, 'do_banshees', 1)
    ...
    def get_nuke_target(self, ghostpos):
        ...
        for tpa in self.enemy_structureinfo:
            ...
            for other_tpa in self.enemy_structureinfo_of_expo[expo]:
                (other_typ, other_pos, other_tag) = other_tpa
        ...
        if self.ghost_phase == 'A':
            ...
            if self.we_started_amount(GHOSTACADEMY) < 1:
                if self.ghost_calmdown == 0:
                    if self.rich():
                        self.throw_somewhere(GHOSTACADEMY, 'ghost_fun', 1)
            ...
            if self.we_started_amount(BARRACKS) < 2:
                if self.ghost_calmdown == 0:
                    if self.rich_nogas():
                        self.throw_somewhere(BARRACKS, 'ghost_fun', 1)
                    self.ghost_calmdown = 80
            else:
                done += 1
            if self.we_started_amount(BARRACKSTECHLAB) < 1:
                if self.rich():
                    self.throw_somewhere(BARRACKSTECHLAB, 'ghost_fun', 1)
                self.ghost_calmdown = 80
            else:
                done += 1
            if self.we_started_amount(FACTORY) < 1:
                if self.ghost_calmdown == 0:
                    if self.rich_nogas():
                        self.throw_somewhere(FACTORY, 'ghost_fun', 1)
                    self.ghost_calmdown = 80
            else:
                done += 1
            # continue
            ...
            if self.we_started_amount(GHOST) < 1:
                if self.ghost_calmdown == 0:
                    if self.rich():
                        self.throw_somewhere(GHOST, 'ghost_fun', 1)
                    self.ghost_calmdown = 80
            else:
                done += 1
            if self.we_started_amount(PERSONALCLOAKING) < 1:
                if self.ghost_calmdown == 0:
                    if self.rich():
                        self.throw_somewhere(PERSONALCLOAKING, 'ghost_fun', 1)
                    self.ghost_calmdown = 80
            else:
                done += 1
    ...
    def get_rushvictims(self,myposition,bound) -> int:
        ...
        for ene in self.enemy_units:  # visible
            if ene.tag in self.enemyworkers:
                if self.near(ene.position, myposition, bound):
                    if reachable == 0:
                        self.rushvictim = ene
                    reachable += 1
    ...
    def defend_spot(self, hospital, enespots):
        # code to attack a few intruders
        n = 0
        sumx = 0
        sumy = 0
        for spot in enespots:
            n += 1
            sumx += spot.x
            sumy += spot.y
        enecenter = Point2((sumx / n, sumy / n))
        ...
        for scv in self.units(SCV):
            if scv.tag in self.speciality_of_tag:
                if self.speciality_of_tag[scv.tag] == 'workerrush':
                    if self.near(scv.position, hospital, 7):
                        emotion = self.emotion_of_unittag[scv.tag]
                        if emotion in self.repairsite_emotions:
                            goodpoint = enecenter.towards(scv.position, wishdist)
                            if not self.near(goodpoint, hospital, self.hospitalsize):
                                goodpoint = hospital.towards(goodpoint, self.hospitalsize)
                            scv.move(goodpoint)
                            self.emotion_of_unittag[scv.tag] = 'ambush'


    def do_workerrush(self):
        if (self.opening_name.find('workerrush') >= 0) and (self.workerrushstate != 'ended'):
            # slow
            if (self.frame > 1100) and (self.frame < 1120):
                self.slowdown_frames = 3000
            # emotions:
            agressive = {'guard','brave','following','jumping','biting','bouncing','firstaid','firstaid2','inaid'}
            defensive = {'fleeing','patient','recovering','inrepair','ambush','berserk','rebound'}
            neutral = {'cured','ingoing'}
            #
            # guard: standing near enemy minerals, to aid, jump or bite.
            # brave: mineralwalking to hisrampmim or hisfarmim. Cooled (<7). Near enemy minerals.
            # following: seeing 1 enemy. Cooled.
            # jumping: near 1 enemy. Cooled. mineralwalking.
            # biting: close to an enemy. Cooled.
            # bouncing: a few frames of mineralwalking back. After biting or if very threatened.
            # firstaid: ready to repair, after bouncing. Hot from biting. (Or in-between state into brave.)
            # firstaid2: same.
            # inaid: going to an own scv or repairing him or being repaired.
            # fleeing: mineralwalking to some mineral. Often low health, will not bite.
            # patient: walking to a hospital.
            # recovering: in a hospital, waiting for an own scv to be repaired. May be repaired.
            # inrepair: in a hospital, going to an own scv or repairing one. May be repaired.
            # ambush: in a hospital, doing an attack dance.
            # berserk: in a hospital, attacking.
            # rebound: in a hospital, a few frames of mineralwalking back.
            # cured: in a hospital, sitting idle with good health.
            # ingoing: mineralwalking to hisrampmim or hisfarmim. Ready to bite. Not near enemy minerals.
            #
            self.repairsite_emotions = {'recovering','inrepair','cured','ambush','berserk','rebound'}
            # minerals to move towards: hisrampmim, hisfarmim, hismidmim,homemim
            ...
            for mim in self.minerals_of_expo[hisexpo]:
                if self.near(mim.position,self.enemy_pos,8):
                    q = self.sdist(mim.position,self.hisleftmim.position) + self.sdist(mim.position,self.hisrightmim.position)
                    if q < lowestq:
                        lowestq = q
                        self.hismidmim = mim
            # homemim
            ...
            for mim in self.minerals_of_expo[myexpo]:
                ...
                if sd < minsd:
                    ...
                    self.homemim = mim
            #
            self.log_success('hisrampmim = '+str(self.hisrampmim.tag))
            self.log_success('hisfarmim = '+str(self.hisfarmim.tag))
            self.log_success('hismidmim = '+str(self.hismidmim.tag))
            self.log_success('homemim = '+str(self.homemim.tag))
            # ramphospital, farhospital, nathospital: a spot not reachable by gather
            x = 0.6 * self.enemyramp_pos.x + 0.4 * self.hisrampmim.position.x
            y = 0.6 * self.enemyramp_pos.y + 0.4 * self.hisrampmim.position.y
            between = Point2((x,y))
            self.ramphospital = between.towards(self.enemy_pos,-2.0)
            # farhospital is at the other side of his main.
            longdist = self.circledist(self.ramphospital,self.hisfarmim.position)
            shortdist = self.circledist(self.ramphospital,self.hisrampmim.position)
            appro = self.enemy_pos
            for step in {1,2,3}:
                appro = self.hisrampmim.position.towards(appro,longdist)
                appro = self.hisfarmim.position.towards(appro,shortdist)
            self.farhospital = appro
            self.nathospital = self.enemynatural_pos.towards(self.loved_pos,3)
            self.turningpoint = self.enemy_pos.towards(self.farhospital,3.5)
            #
                    ...
                    for scv in self.units(SCV):
                        ...
                        if job in self.bad_jobs + self.no_jobs:
                            if todo > 0:
                                ...
                                self.defencespot[scv.tag] = self.ramphospital
                                self.wait_of_unittag[scv.tag] = 0 # for emotions
                            else:
                                # give fresh mining place, especially on golden wall
                                self.promote(scv,'escorter')
                                scv.attack(self.loved_pos.towards(self.game_info.map_center,-3))
            ...
            if self.stackphase == 'A':
                ...
                for scv in self.units(SCV):
                    if scv.tag in self.speciality_of_tag:
                        if self.speciality_of_tag[scv.tag] == 'workerrush':
                            dist = self.circledist(scv.position,back.position)
                            if (dist > 40): # neglect late arrivers
                                if dist < mindist:
                                    mindist = dist
                                    self.stacktag = scv.tag
            ...
            for scv in self.units(SCV):
                if scv.tag in self.speciality_of_tag:
                    if self.speciality_of_tag[scv.tag] == 'workerrush':
                        ...
                        for ene in self.enemy_units:  # visible
                            if ene.tag in self.enemyworkers:
                                dist = self.circledist(scv.position,ene.position)
                                dte = min(dte,dist)
                        wai = self.wait_of_unittag[scv.tag]
                        self.log_success(self.name(scv.tag) + ' ' + str(scv.health) + ' ' + emotion
                                         + ' ' + str(scv.weapon_cooldown) + ' ' + str(dte) + ' ' + str(wai))
            # delete rushrepair
            todel = set()
            ...
            for (aider_tag,aidee_tag) in self.rushaids:
                ...
                for may_aider in self.units(SCV):
                    if may_aider.tag == aider_tag:
                        if self.emotion_of_unittag[aider_tag] == 'inaid':
                            aiderfound = True
                            aider = may_aider
                if aiderfound:
                    aideefound = False
                    for aidee in self.units(SCV):
                        if aidee.tag == aidee_tag:
                            if self.emotion_of_unittag[aidee.tag] == 'inaid':
                                aideefound = True
            ...
            self.rushaids -= todel
            for (aider_tag,aidee_tag) in self.rushaids:
                self.log_success(self.name(aider_tag)+' is aiding '+self.name(aidee_tag))
            # counts
            agressivecount = 0
            defensivecount = 0
            neutralcount = 0
                ...
                if scv.tag in self.speciality_of_tag:
                    if self.speciality_of_tag[scv.tag] == 'workerrush':
                        ...
                        if emotion in agressive:
                            ...
                            defensivecount += 1
            # enemyworkers: tag of reachable enemy workers
            # not overlord, not inside a building structure
            self.enemyworkers = set()
            for ene in self.enemy_units:  # visible
                if ene.type_id not in {EGG,LARVA,OVERLORD}:
                    outside = True
                    for stru in self.enemy_structures:
                        if self.near(ene.position,stru.position,stru.radius):
                            outside = False
                    if outside:
                        self.enemyworkers.add(ene.tag)
            ...
            for scv in self.units(SCV):
                if scv.tag in self.last_health:
                    if scv.health < self.last_health[scv.tag]:
                        ...
                        for ene in self.enemy_units:  # visible
                            if ene.tag in self.enemyworkers:
                                if self.near(ene.position,scv.position,self.bitebound): # 0.85 theory
            ...
            # enemies in the ramphospital
            enespots = set()
            for ene in self.enemy_units:  # visible
                if ene.tag in self.enemyworkers:
                    if self.near(ene.position, self.ramphospital, self.hospitalsize):
                        enespots.add(ene.position)
            if len(enespots) == 0:
                for scv in self.units(SCV):
                    if scv.tag in self.speciality_of_tag:
                        if self.speciality_of_tag[scv.tag] == 'workerrush':
                            if self.near(scv.position, self.ramphospital, 6):
                                emotion = self.emotion_of_unittag[scv.tag]
                                if emotion in {'ambush','berserk'}:
                                    self.emotion_of_unittag[scv.tag] = 'recovering'
            else:
                self.defend_spot(self.ramphospital,enespots)
            # enemies in the farhospital
            enespots = set()
            for ene in self.enemy_units:  # visible
                if ene.tag in self.enemyworkers:
                    if self.near(ene.position, self.farhospital, self.hospitalsize):
                        enespots.add(ene.position)
            if len(enespots) == 0:
                for scv in self.units(SCV):
                    if scv.tag in self.speciality_of_tag:
                        if self.speciality_of_tag[scv.tag] == 'workerrush':
                            if self.near(scv.position, self.farhospital, 6):
                                emotion = self.emotion_of_unittag[scv.tag]
                                if emotion in {'ambush','berserk'}:
                                    self.emotion_of_unittag[scv.tag] = 'recovering'
            else:
                self.defend_spot(self.farhospital,enespots)
            # enemies in the nathospital
            enespots = set()
            for ene in self.enemy_units:  # visible
                if ene.tag in self.enemyworkers:
                    if self.near(ene.position, self.nathospital, self.hospitalsize):
                        enespots.add(ene.position)
            if len(enespots) == 0:
                for scv in self.units(SCV):
                    if scv.tag in self.speciality_of_tag:
                        if self.speciality_of_tag[scv.tag] == 'workerrush':
                            if self.near(scv.position, self.nathospital, 6):
                                emotion = self.emotion_of_unittag[scv.tag]
                                if emotion in {'ambush','berserk'}:
                                    self.emotion_of_unittag[scv.tag] = 'recovering'
            else:
                self.defend_spot(self.nathospital,enespots)
            #
            for scv in self.units(SCV):
                if scv.tag in self.speciality_of_tag:
                    if self.speciality_of_tag[scv.tag] == 'workerrush':
                        ...
                        for (mim,hos) in {(self.hisfarmim,self.farhospital),(self.hisrampmim,self.ramphospital),
                                          (self.homemim,self.ramphospital)}:
                            enem = 0
                            safetycenter = scv.position.towards(mim.position,4)
                            for ene in self.enemy_units:  # visible
                                if ene.tag in self.enemyworkers:
                            ...
                            if enem < enemies_behind_you:
                                ...
                                awaymim = mim
                                awaygoal = hos
                        if self.near(scv.position,awaymim.position,2):
                            awaymim = self.homemim
                            awaygoal = self.ramphospital
                        # bittencount
                        bittencount = 0
                        for ene in self.enemy_units:  # visible
                            if ene.tag in self.enemyworkers:
                                if self.near(ene.position, scv.position, self.bitebound):
                                    if self.known_hit_by_enemy[ene.tag] < self.frame - 18:
                                        bittencount += 1
                        #
                        followvictims = self.get_rushvictims(scv.position, self.followbound)
                        #
                        self.wait_of_unittag[scv.tag] -= self.chosen_game_step
                        # timed: bouncing, rebound, berserk, following, jumping, biting
                        #
                        ...
                        if (bittencount > 2):
                            if emotion in agressive:
                                ...
                                emotion = 'rebound'
                        # go flee
                        minhealthbound = 5.5 + enemies_behind_you * 4
                        minhealthbound = min(20.5,minhealthbound)
                        ineedyou = min(4,len(self.enemyworkers)-1)
                        if (scv.health < minhealthbound):
                            if emotion in agressive | neutral:
                                emotion = 'fleeing'
                                self.defencespot[scv.tag] = awaygoal
                                self.towardsmineral[scv.tag] = awaymim
                                self.last_sd_of_unittag[scv.tag] = 99999
                        if (agressivecount + neutralcount <= ineedyou):
                            if emotion in agressive | neutral:
                                emotion = 'fleeing'
                                self.defencespot[scv.tag] = awaygoal
                                self.towardsmineral[scv.tag] = awaymim
                        ...
                        #
                        if emotion == 'fleeing':
                            goal = self.defencespot[scv.tag]
                            sd = self.sdist(scv.position,goal)
                            if sd >= self.last_sd_of_unittag[scv.tag]:
                                emotion = 'patient'
                            self.last_sd_of_unittag[scv.tag] = sd
                            ...
                            if emotion != 'guard':
                                if (emotion in agressive):
                                    ...
                                    emotion = 'rebound'
                        elif (self.get_rushvictims(scv.position,self.bitebound) > 0):
                            if (scv.weapon_cooldown < 5):
                                if emotion == 'ambush':
                                    emotion = 'berserk'
                                else:
                                    emotion = 'biting'
                        elif (self.get_rushvictims(scv.position, jumpbound) > 0):
                            ...
                            if self.near(scv.position, tomim.position, 8):
                                emotion = 'brave'
                        elif emotion == 'bouncing':
                            if self.wait_of_unittag[scv.tag] <= 0:
                                emotion = 'firstaid'
                        elif emotion == 'rebound':
                            if self.wait_of_unittag[scv.tag] <= 0:
                                emotion = 'recovering'
                        elif emotion == 'patient':
                            goal = self.defencespot[scv.tag]
                            if self.near(scv.position, goal, self.hospitalsize):
                                emotion = 'recovering'
                        elif emotion == 'cured':
                            localdefensivecount = 0
                            localneutralcount = 0
                            for otherscv in self.units(SCV):
                                if otherscv.tag in self.speciality_of_tag:
                                    if self.speciality_of_tag[otherscv.tag] == 'workerrush':
                                        emotion = self.emotion_of_unittag[otherscv.tag]
                                        if self.near(otherscv.position, scv.position, 7):
                                            if emotion in defensive:
                                                localdefensivecount += 1
                                            elif emotion in neutral:
                                                localneutralcount += 1
                            if scv.health < self.healthybound: # can happen
                                emotion = 'recovering'
                            elif (localdefensivecount == 0) and (localneutralcount >= 4):
                                emotion = 'ingoing'
                            elif (defensivecount == 0):
                                emotion = 'ingoing'
                        elif emotion == 'berserk':
                            if self.wait_of_unittag[scv.tag] <= 0:
                                emotion = 'ambush'
                        elif emotion in {'following','jumping','biting'}:
                            if self.wait_of_unittag[scv.tag] <= 0:
                                emotion = 'firstaid'
                        elif emotion == 'recovering':
                            if scv.health >= self.healthybound:
                                emotion = 'cured'
                        elif emotion in {'firstaid','firstaid2','inaid'}:
                            isaider = False
                            for (aidertag,aideetag) in self.rushaids:
                                if scv.tag == aidertag:
                                    isaider = True
                            if not isaider:
                                minsd = 99999
                                for otherscv in self.units(SCV):
                                    if otherscv.tag != scv.tag:
                                        if otherscv.tag in self.speciality_of_tag:
                                            if self.speciality_of_tag[otherscv.tag] == 'workerrush':
                                                if self.emotion_of_unittag[otherscv.tag] in {'guard','brave','firstaid','firstaid2','inaid'}:
                                ...
                                if minsd < 3*3:
                                    if self.minerals > 5:
                                        emotion = 'inaid'
                                        self.emotion_of_unittag[bestscv.tag] = 'inaid' # he too
                                        self.rushaids.add((scv.tag, bestscv.tag))
                            if not self.near(scv.position,self.hisfarmim.position,8):
                                # move back
                                if emotion == 'firstaid':
                                    emotion = 'firstaid2'
                                elif emotion == 'firstaid2':
                                    emotion = 'firstaid'
                        # try to repair
                        if emotion in {'recovering','cured'}:
                            ...
                            for otherscv in self.units(SCV):
                                if otherscv.tag != scv.tag:
                                    if otherscv.tag in self.speciality_of_tag:
                                        if self.speciality_of_tag[otherscv.tag] == 'workerrush':
                                            if self.emotion_of_unittag[otherscv.tag] in {'recovering','inrepair'}:
                                                if otherscv.health < self.healthybound:
                                                    sd = self.sdist(scv.position,otherscv.position)
                                                    if sd < minsd:
                                                        minsd = sd
                                                        bestscv = otherscv
                            if minsd < 99999:
                                if (minsd < 6*6) or (self.frame % 43 == 42):
                                    if self.minerals > 20:
                                        emotion = 'inrepair'
                                        self.emotion_of_unittag[otherscv.tag] = 'inrepair'
                                        self.rushrepairs.add((scv.tag,bestscv.tag))
                        #
                        if (emotion in {'firstaid','firstaid2'}) and (scv.weapon_cooldown < 7):
                            emotion = 'brave'
                        #
                        ...
                        if emotion != last_emotion[scv.tag]:
                            if emotion in {'bouncing','rebound'}:
                                self.wait_of_unittag[scv.tag] = 10 - scv.health / 7
                            elif emotion in {'berserk','following','jumping','biting'}:
                                self.wait_of_unittag[scv.tag] = 7
                        #
                        # workerrush move
                        re_move = False
                        if emotion != last_emotion[scv.tag]:
                            ...
                            re_move = True
                        elif self.near(scv.position, self.hisfarmim.position, 2):
                            re_move = True
                            ...
                            re_move = True
                        if re_move:
                            if emotion in {'biting','berserk'}:
                                scv.attack(self.rushvictim)
                            elif emotion == 'jumping':
                                maxcool = -99999
                                for mim in {self.homemim, self.hismidmim, self.hisrampmim, self.hisfarmim}:
                                    cool = self.circledist(scv.position, mim.position) - self.circledist(
                                        self.rushvictim.position, mim.position)
                                    if cool > maxcool:
                                        maxcool = cool
                                        bestmim = mim
                                scv.gather(bestmim)
                            elif emotion == 'following':
                                scv.move(self.rushvictim.position)
                            elif emotion == 'brave':
                                if self.near(scv.position, self.hisfarmim.position, 2):
                                    self.towardsmineral[scv.tag] = self.hisrampmim
                                elif self.near(scv.position, self.hisrampmim.position, 2):
                            ...
                            elif emotion in {'bouncing','rebound'}:
                                scv.gather(awaymim)
                            elif emotion == 'fleeing':
                                if self.near(scv.position, self.hisfarmim.position, 2):
                                    ...
                                    scv.gather(self.towardsmineral[scv.tag])
                            elif emotion in {'patient','recovering','cured'}:
                                goal = self.defencespot[scv.tag]
                                scv.move(goal)
                            elif emotion in {'firstaid','firstaid2'}:
                                if self.near(scv.position, self.hisrampmim.position, 2):
                                    scv.gather(self.homemim)
                                else:
                                    scv.gather(self.hisrampmim)
                            elif emotion in {'inaid','inrepair'}:
                                itisarepairer = False
                                        ...
                                        for repairee in self.units(SCV):
                                            if repairee.tag == repairee_tag:
                                                if self.minerals >= 10:
                                                    ...
                                                    scv.move(repairee.position)
                                if not itisarepairer: # it is a reparee
                                    scv.move(scv.position.towards(self.loved_pos,0.1)) # is it a bug to go to own position?
                            elif emotion == 'ingoing':
                                if self.sdist(scv.position, self.hisfarmim.position) > 20:
                                    tomim = self.hisrampmim
                                elif self.sdist(scv.position,self.hisfarmim.position) > self.sdist(scv.position,self.hisrampmim.position):
                                    tomim = self.hisrampmim
                                else:
            ...
            if self.cheese3_phase == 'A':
                ...
                for bunpos in {self.cheese3_bunker_pos,self.cheese3_bunker2_pos}:
                    if not self.we_started_at(BUNKER,bunpos):
                        self.throw_at_spot(BUNKER,bunpos,'do_cocoon',1)
                for bu in self.structures(BUNKER):
                    if bu.tag not in bunker_if.door:
                ...
                for ba in self.structures(BARRACKS):
                    if ba.position == self.cheese1_landing_pos:
                        self.throw_at_spot(MARINE, ba.position, 'do_cocoon', 1)
                        for scv in self.units(SCV):
                            if scv.tag == self.scout1_tag:
                                self.promote(scv, 'idler')
                                self.scout1_tag = -2 # notag, but notag would rerecruite it
                        self.throw_at_spot(BUNKER, self.cheese1_bunker_pos, 'do_cocoon', 1)
                ...
                if self.we_finished_a(ENGINEERINGBAY):
                    ...
                    goal = self.place_around(MISSILETURRET, goal)
                    self.throw_at_spot(MISSILETURRET,goal,'do_cocoon',1)
                ...
                if bu.health < 500:
                    if bu.tag not in self.speciality_of_tag:
                        if not bu.is_idle:
                            ...
                            self.log_command('bu(AbilityId.CANCEL_QUEUECANCELTOSELECTION)') # build worker
                            bu(AbilityId.CANCEL_QUEUECANCELTOSELECTION)
                        self.landings_of_flying_struct[bu.tag] = 0
...
# worker routines
                ...
                if low_qual == 1.0:
                    for s in self.units(SCV):
                        if s.tag in self.job_of_scvt:
                            ...
                            if (job in self.good_jobs) and (job != 'scout'):
                                qual = s.health / self.maxhealth[s.type_id]
                                if qual < low_qual:
                                    expo = self.expo_of_pos(s.position)
                                    hasrepairers = 0
                ...
                for mim in self.minerals_of_expo[expo]:
                    if mim.tag == assigned_mimt:
                        if scvt in self.minedirection_of_scvt:
                            # when starting to return, no conclusions when speedmining
                    ...
                    if gas.tag == assigned_gast:
                        if scvt in self.minedirection_of_scvt:
                            if scv.is_returning and (self.minedirection_of_scvt[scvt] == 'G'):
                                ...
                                if not self.near(gas.position, scv.position, 6):
    ...
    async def speedmining(self):
        # does:           detail mineralminers walk
        # input:          self.mimt_of_scvt    defines the mining relations, regulated elswhere.
        # uses:           all_bases            landed finished halls
        # private use:    _of_minert
        if self.supply_used >= 100:
            # No speedmining. Miners should mine.
            for scv in self.units(SCV).idle:
                scvt = scv.tag
                if scvt in self.mimt_of_scvt:
                    mimt = self.mimt_of_scvt[scvt]
                    for mim in self.mineral_field:
                        if mim.tag == mimt:
                            scv(AbilityId.HARVEST_GATHER_SCV, mim)
        else:
            # speedmining. Miners may move or stand still.
            # speedbases
            if len(self.all_bases) != self.speedbases:
                self.speedbases = len(self.all_bases)
                for scvt in self.mimt_of_scvt:
                    mimt = self.mimt_of_scvt[scvt]
                    for mim in self.mineral_field:
                        if mim.tag == mimt:
                            ...
                            for base in self.all_bases:
                                ...
                                if sd < bestsd:
                                    bestsd = sd
                                    bestbasepos = basepos
                            self.basepos_of_minert[scvt] = bestbasepos
            # _of_minert
            todel = set()
            for scvt in self.mimtag_of_minert:
                if scvt not in self.mimt_of_scvt:
                    todel.add(scvt)
            for scvt in todel:
                del self.mimtag_of_minert[scvt]
                del self.mimpos_of_minert[scvt]
                del self.phase_of_minert[scvt]
                del self.count_of_minert[scvt]
                del self.lastdist_of_minert[scvt]
                del self.basepos_of_minert[scvt]
            for scvt in self.mimt_of_scvt:
                mimt = self.mimt_of_scvt[scvt]
                toconnect = False
                if scvt in self.mimtag_of_minert:
                    if self.mimtag_of_minert[scvt] != mimt:
                        toconnect = True
                else:
                    toconnect = True
                if toconnect:
                    for mim in self.mineral_field:
                        if mim.tag == mimt:
                            for scv in self.units(SCV):
                                if scv.tag == scvt:
                                    self.mimtag_of_minert[scvt] = mimt
                                    self.mimpos_of_minert[scvt] = mim.position
                                    self.phase_of_minert[scvt] = 'A' # wrong start is no problem
                                    self.count_of_minert[scvt] = 0
                                    dist = self.circledist(mim.position, scv.position)
                                    self.lastdist_of_minert[scvt] = dist
                                    # basepos
                                    bestsd = 99999
                                    bestbasepos = self.nowhere
                                    for base in self.all_bases:
                                        basepos = base.position
                                        sd = self.sdist(basepos, mim.position)
                                        if sd < bestsd:
                                            bestsd = sd
                                            bestbasepos = basepos
                                    self.basepos_of_minert[scvt] = bestbasepos
            ...
            for scv in self.units(SCV):
                scvt = scv.tag
                if (scvt in self.mimtag_of_minert):
                    # await self.debug(scv)
                    gathermintime = 52
                    mimpos = self.mimpos_of_minert[scvt]
                    basepos = self.basepos_of_minert[scvt]
                    radius = self.circledist(mimpos,basepos)
                    mimt = self.mimtag_of_minert[scvt]
                    dist = self.circledist(mimpos,scv.position)
                    maxclick = basepos.towards(self.game_info.map_center,3)
                    waitposition = mimpos.towards(basepos,3)
                    waitdist = 3
                    if mimt not in self.minclick_of_mimt:
                        self.minclick_of_mimt[mimt] = mimpos.towards(self.game_info.map_center,-1)
                    minclick = self.minclick_of_mimt[mimt]
                    # log
                    self.log_speedmining(self.name(scvt)+' dist '+str(dist)+' '+str(radius)+'   '+str(self.count_of_minert[scvt])
                                    +'   '+str(dist-self.lastdist_of_minert[scvt])+' '+self.phase_of_minert[scvt])
                    self.count_of_minert[scvt] += self.chosen_game_step
                    last_phase = self.phase_of_minert[scvt]
                    new_phase = last_phase
                    # phase change by position
                    if last_phase in {'G','R','I'}:
                        if (dist < self.lastdist_of_minert[scvt]) and (dist>=2):
                            new_phase = 'G'
                        elif (dist > self.lastdist_of_minert[scvt]) and (dist>=2):
                            new_phase = 'R'
                        elif (dist == self.lastdist_of_minert[scvt]) and (dist<2):
                            new_phase = 'I'
                    # phase change by condition
                    if last_phase == 'A':
                        if dist < waitdist + 0.7:
                            for altscvt in self.mimtag_of_minert:
                                if self.mimtag_of_minert[altscvt] == mimt:
                                    if self.count_of_minert[altscvt] < gathermintime:
                                        if altscvt != scvt:
                                            for altscv in self.units(SCV):
                                                if altscv.tag == altscvt:
                                                    altdist = self.circledist(mimpos, altscv.position)
                                                    if altdist < dist:
                                                        new_phase = 'W'
                        if dist < waitdist:
                            new_phase = 'G'
                        ...
                        if dist > self.lastdist_of_minert[scvt]:
                            new_phase = 'R'
                    elif last_phase == 'R':
                        if dist > radius - 4.0:
                            if radius < 7.0:
                        ...
                        if dist < self.lastdist_of_minert[scvt]:
                            new_phase = 'A'
                    # phase change by timeout
                    if (radius < 8.0) and (self.frame > 250):
                        maxtime = [20,26,80,88,112,116,128,144,150]
                        ix = 'GHIJRSTAW'.find(last_phase)
                        if self.count_of_minert[scvt] > maxtime[ix]:
                            new_phase = 'HIJRSTAWG'[ix]
                            self.log_success('Warning: a speedminer got a timeout in '+last_phase)
                    #
                    # actions
                    if new_phase != last_phase:
                        if new_phase == 'W':
                            scv.move(waitposition)  # to prevent clash
                        if last_phase == 'W':
                            for mim in self.mineral_field:
                                if mim.tag == mimt:
                                    scv(AbilityId.HARVEST_GATHER_SCV, mim)
                        if new_phase == 'G':
                            self.log_speedmining('round ' + str(self.count_of_minert[scvt]))
                            self.count_of_minert[scvt] = 0
                        if new_phase == 'H':
                            scv.move(minclick)
                        if last_phase == 'H':
                            for mim in self.mineral_field:
                                if mim.tag == mimt:
                                    scv(AbilityId.HARVEST_GATHER_SCV, mim)
                        if new_phase == 'J':
                            for hall in self.all_bases:
                                if hall.position == basepos:
                                    scv(AbilityId.SMART, hall)
                        if new_phase == 'S':
                            scv.move(maxclick)
                        if last_phase == 'S':
                            scv(AbilityId.HARVEST_RETURN_SCV)
                    # middle-minerals
                    if (last_phase == 'H') and (radius < 8.0) and (self.count_of_minert[scvt] >= 18) and (self.frame > 125):
                        self.minclick_of_mimt[mimt] = mimpos.towards(basepos,0.9)
                    # store for next time
                    self.phase_of_minert[scvt] = new_phase
                    self.lastdist_of_minert[scvt] = dist
    ...
    async def win_loss(self):
        ...
        if self.game_result == 'doubt':
            ...
            if self.supply_used>190:
                ...
                self.log_success('good opening')
                for nr in range(0,self.game_choices):
                    if self.game_choice[nr]:
                        self.strategy[spec][nr] = 0.85*self.strategy[spec][nr]+0.15
                    else:
                        self.strategy[spec][nr] *= 0.85
            elif max_cc_health < 500:
                self.game_result = 'loss'
                self.log_success('bad opening')
                for nr in range(0, self.game_choices):
                    addum = 0.15
                    if nr < self.radio_choices:
                        addum = 0.15 / (self.radio_choices - 1)
                    if self.game_choice[nr]:
                        self.strategy[spec][nr] *= 0.85
                    else:
                        self.strategy[spec][nr] = 0.85*self.strategy[spec][nr]+addum
            # write
            if self.game_result in ['win','loss']:
                # min 0.01, max 0.99
                for nr in range(0, self.game_choices):
                    self.strategy[spec][nr] = max(self.strategy[spec][nr], 0.01)
                    self.strategy[spec][nr] = min(self.strategy[spec][nr], 0.99)
                for nr in range(0, self.radio_choices):
                    self.strategy[spec][nr] = min(self.strategy[spec][nr], 0.30)
                # radio tuning
                sum = 0.0
                for nr in range(0, self.radio_choices):
                    sum = sum + self.strategy[spec][nr]
