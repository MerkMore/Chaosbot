...
class Chaosbot(sc2.BotAI):
    ...
    all_minerals = set() # mineral_field but with a stable tag
    ...
    good_jobs = ('builder','settler','fencer','traveller','repairer','fighter','escorter','scout','cheeser','pilot')
    bad_jobs = ('gasminer','mimminer','candidate','carrier','applicant','gasper','reporter')
    no_jobs = ('idler','suicider','volunteer','inshock','fleeer')
    #
    jobs_may_idle = ('mimminer','fencer','settler','scout','fighter','cheeser','applicant','idler','gasper','pilot')
    #
    expected_orders = set()
    #
    job_of_scvt = {}
    count_of_job = {}
    #   applicants and candidates have a vision, a tag of a base to walk to
    vision_of_scvt = {}
    ...
    amount_usable = {} # per buildingtype eg barracks, count of barracks (at least half-built), minus count of marines etc in throwspots
    #
    ...
    async def my_init(self):
        ...
        if self.enemy_race != Race.Zerg:
            ...
            self.enemy_structureinfo_plus.add((HATCHERY,self.enemy_pos,self.notag,0))
        elif self.enemy_race != Race.Protoss:
            self.enemy_structureinfo.add((NEXUS, self.enemy_pos, self.notag))
            self.enemy_structureinfo_plus.add((NEXUS, self.enemy_pos, self.notag,0))
        else: # random or terran
            self.enemy_structureinfo.add((COMMANDCENTER, self.enemy_pos, self.notag))
            self.enemy_structureinfo_plus.add((COMMANDCENTER, self.enemy_pos, self.notag,0))
        # vulture
        self.init_vulture()
            ...
            if minert not in self.speciality_of_tag:
                ...
                for mim in self.all_minerals:
                    ...
                    if self.count_of_mimt[mimt] < 2:
                        sd = self.sdist(miner.position,mimpos) + 30 * self.count_of_mimt[mimt]
                        if sd < best_sd:
                            best_sd = sd
                            bestmim = mim
                mim = bestmim
                (mimpos,mimt) = mim
                self.mimt_of_minert[minert] = mimt
                self.count_of_mimt[mimt] += 1
        # gather (unless second)
        for miner in self.units(SCV):
            minert = miner.tag
            if minert in self.mimt_of_minert:
                gogather = True
                thismimt = self.mimt_of_minert[minert]
                if self.count_of_mimt[thismimt] == 2:
                    # smallest tag will mine first
                    for otherminert in self.mimt_of_minert:
                        if self.mimt_of_minert[otherminert] == thismimt:
        ...
        # chat
        await self._client.chat_send('Chaosbot version 26 sep 2021, made by MerkMore', team_only=False)
        #
    ...
    def mineraltag(self, patch) -> int:
        return round(1000*patch.position.x+2*patch.position.y)
    ...
    def init_all_minerals(self):
        if len(self.mineral_field) != self.mineral_field_hash:
            ...
            self.all_minerals = {(patch.position,self.mineraltag(patch)) for patch in self.mineral_field}

    def go_gather(self, scv, mim):
        (mimpos,mimt) = mim
        scvt = scv.tag
        done = False
    ...
    def init_marine_opening(self, nbases):
        ...
        self.opening_create_units = nbases * 28 # 4 barracks * 7 marines/barracks
        self.opening_create_kind = MARINE
                ...
                if len(scv.orders) == 2:
                    ...
                    if job == 'mimminer':
                        if scv.orders[0].ability.id == AbilityId.HARVEST_RETURN:
                            if scv.orders[1].ability.id == AbilityId.HARVEST_RETURN: # bug?
                                normal = True
                if (len(scv.orders) == 1):
    ...
    def allow_throw(self, thing) -> bool:
        # throw max 2 parallel per category
        # also admit waitfree throws
        allow = False
        if self.check_techtree(thing):
            if self.opening_create_units + len(self.buildorder_exe) < 6:
                ...
                if thing not in self.all_structures_tobuildwalk:
                    if self.can_pay(thing):
                        ...
                        if len(self.structures(crad).ready.idle) > 0:
                            allow = True
        return allow
    ...
    def fix_count_of_mimt(self):
        #       calculate the amount of gatherers on each mineral, using mimt_of_minert
        self.count_of_mimt = {}
        for mimt in self.all_mimt:
            self.count_of_mimt[mimt] = 0
        for scvt in self.mimt_of_minert:
            mimt = self.mimt_of_minert[scvt]
    ...
    def mimminer_vacatures(self) -> int:
        return self.maxmimminers() - len(self.mimt_of_minert)
    ...
    def get_near_mimt_free(self,point) -> int:
        ...
        for (mimpos,mimt) in self.minerals_of_expo[expo]:
            if mimt in self.all_mimt:
                if self.count_of_mimt[mimt] < 2:
                    sd = self.sdist(mimpos,point) + 30 * self.count_of_mimt[mimt]
                    if sd < best_sdist:
                        best_sdist = sd
                        best_mimt = mimt
        if best_sdist == 99999:
        ...
        for pair in self.cradle:
            ...
            for bar in self.structures(bartype).ready:
                if bar.is_using_ability(abi):
                    ...
                    # the TECHLAB progress bugs. Thus extra own administration.
                    ...
                    if martype in self.all_labs:
                        ...
                        for (tag,startpos,startframe) in self.labprogress_buildframes:
                            if (tag == bar.tag) and (startpos == bar.position):
                                seen = True
                                progress = (self.frame - startframe) / (18 * 22.4)
                                if progress > 1.2:
                                    seen = False
                                    progress = 0
...
#*********************************************************************************************************************
    ...
    async def gamestate(self):
        ...
        self.fix_count_of_job()
        self.log_population()
        #   restrict mimt_of_minert to existing worker and mineral
        new_mimt_of_minert = {}
        for scvt in self.mimt_of_minert:
            if scvt in self.all_scvt:
                mimt = self.mimt_of_minert[scvt]
                if mimt in self.all_mimt:
                    new_mimt_of_minert[scvt] = mimt
        for scvt in set(self.mimt_of_minert) - set(new_mimt_of_minert):
            self.log_workers('from mimt_of_minert gone is mimt_of_'+str(scvt))
        self.mimt_of_minert = new_mimt_of_minert
        ...
        for scvt in self.mimt_of_minert:
            ...
            name = self.name(scvt)
            if (job != 'mimminer'):
                self.log_success('not a mimminer, yet in mimt_of_minert '+job+' '+name)
        for scvt in self.gast_of_scvt:
            job = self.job_of_scvt[scvt]
            name = self.name(scvt)
            if (job != 'gasminer'):
        ...
        for ene in self.enemy_structures:
            ...
            if goodtpa not in self.enemy_structureinfo:
                ...
                tpaplus = (ene.type_id,ene.position,ene.tag,finishframe)
                self.enemy_structureinfo_plus.add(tpaplus)
        if itchanged:
            # delete from tpaplus
            if len(todel) > 0:
                for tpaplus in self.enemy_structureinfo_plus:
                    (typ, pos, tag, finishframe) = tpaplus
                    if (typ, pos, tag) in todel:
                        todelplus.add(tpaplus)
        ...
        for kind in {BARRACKS,FACTORY,STARPORT,ARMORY,ENGINEERINGBAY}:
            ...
            for bar in self.structures(kind).ready:
                self.amount_usable[kind] += 1
            for (marty,bartype,pos,dura) in self.eggs:
                if (marty == kind) and (dura < 20):
                    self.amount_usable[kind] += 1
            for (thing, pos, status, owner) in self.throwspots:
    ...
    def throw_at_spot(self, th, po, ow):
        ...
        approve = True
        for kind in {BARRACKS,FACTORY,STARPORT,ARMORY,ENGINEERINGBAY}:
            if self.cradle_of_thing(th) == kind:
    ...
    async def throw_advance(self):
        ...
        if self.opening_create_units > 0:
            cost = self.get_added_cost(self.opening_create_kind)
            save_bo_sum_minerals += cost.minerals * self.opening_create_units
            save_bo_sum_vespene += cost.vespene * self.opening_create_units
            save_opening_minerals += cost.minerals * self.opening_create_units
            save_opening_vespene += cost.vespene * self.opening_create_units
        for (thing, place, status) in self.buildorder_exe:
            cost = self.get_added_cost(thing)
    ...
    def army_adlib(self):
        ...
        if self.opening_create_units > 0:
            ...
            avoid_pos = set()
            for (item, goal, status) in self.buildorder_exe:
                avoid_pos.add(goal) # this barracks wants a lab
            for tps in self.throwspots:
                (th, po, status, ow) = tps
                avoid_pos.add(po) # this barracks will make a marine already
                            ...
                            if bar.has_techlab or (self.opening_create_kind not in self.all_labarmy):
                                if bar.position not in avoid_pos:
                                    if self.can_pay(self.opening_create_kind):
                                        if units_todo > 0:
                                            self.throw_at_spot(self.opening_create_kind, bar.position, 'army_adlib')
                                            units_todo -= 1
        else: # normal
            if ('army' in self.good_plans):
    ...
    def free_normalbasepositions(self) -> int:
        freenormals = 0
        for normalpos in self.expansion_locations:
            ...
            for stru in self.structures:
                ...
                if self.near(pos, normalpos, 10):
                    free = False
            for (kind, pos, tag) in self.enemy_structureinfo:
                if self.near(pos, normalpos, 10):
                    free = False
            for (th, pos, status, ow) in self.throwspots:
                if th == COMMANDCENTER:
                    ...
                    if self.cc_destiny[cc.position] == 'pf':
                        if not self.we_started_at(PLANETARYFORTRESS, cc.position):
                            if self.allow_throw(PLANETARYFORTRESS):
                                self.throw_at_spot(PLANETARYFORTRESS, cc.position, 'pfoc_adlib')
                    elif not self.we_started_at(ORBITALCOMMAND, cc.position):
                        if self.allow_throw(ORBITALCOMMAND):
                            self.throw_at_spot(ORBITALCOMMAND, cc.position, 'pfoc_adlib')

    def cc_adlib(self):
        if ('base' in self.good_plans):
            if self.allow_throw(COMMANDCENTER):
                if self.free_normalbasepositions() >= 1:
                    self.throw_anywhere(COMMANDCENTER, 'cc_adlib')

    def chaosbases_adlib(self):
        if self.allow_throw(COMMANDCENTER):
            if self.free_normalbasepositions() < 4:
    ...
    def prepped_to_egg(self,  martype, bartype, pos):
        ...
        if self.opening_create_kind == martype:
            if self.opening_create_units > 0:
        ...
        self.preps -= todel
        if len(todel) != 1:
            print('BUG prepped_to_egg ' + martype.name + ' ' + str(len(todel)))
        # del throw
        throwtodel = set()
        for tps in self.throwspots:
            (th, po, status, ow) = tps
...
#   army movement routines
    ...
    def init_undetect(self):
        ...
        hash = 0
        for tpa in self.enemy_structureinfo_plus:
    ...
    async def marine_fun(self):
        if self.opening_name.find('marine') >= 0:
            ...
            if self.we_started_a(STIMPACK):
                self.throw_treeno(FACTORY, 'marine_fun')
            if self.we_started_a(FACTORY):
                self.throw_treeno(STARPORT, 'marine_fun')
            if self.we_started_a(STARPORT):
                self.throw_treeno(FUSIONCORE, 'marine_fun')
            # start the attack
            ...
            if self.frame % 7 == 6:
                if self.frame < self.marine_gatherframe + 22.4 * 60:
                    for mar in self.units(MARINE):
                        if mar.tag not in self.speciality_of_tag:
                            mar.attack(self.marine_goal)
                            self.speciality_of_tag[mar.tag] = self.opening_name
                        ...
                        if order.ability.id == AbilityId.ATTACK:
                            if type(order.target) == int:
                                for ene in self.enemy_units: # actual visible
                                    if ene.tag == order.target:
                                        if ene.type_id in {LARVA,EGG,OVERLORD,OVERSEER}:
    ...
    def fly_viking(self):
        ...
        # a viking is (repairing or looking, with squadron -1) or (it is in a squadron).
        for vik in self.units(VIKINGFIGHTER):
    ...
    async def attack_with_rest(self):
            ...
            for scv in self.units(SCV).idle:
                ...
                if scvt in self.all_scvt:
                    if self.job_of_scvt[scvt] == 'suicider':
                        (place,tag) = random.choice(tuple(self.all_minerals)) # mined out
        ...
        for mar in self.units(MARINE):
            if mar.tag not in self.speciality_of_tag:
                if mar.tag not in bunker_if.marinetags:
                    (tp, tag) = random.choice(tuple(self.all_minerals))  # mined out
        ...
        if self.opening_name.find('marauders') >= 0:
            ...
            if (reached >= 5):
                (self.marauder_goal, tag) = random.choice(tuple(self.all_minerals))  # mined out
                allloc = []
                for tpa in self.enemy_structureinfo:
                    (typ, pos, tag) = tpa
                    allloc.append(pos)
                ...
                if len(points) == 0:
                    for (mimpos,mimt) in self.all_minerals:
                        if self.proxy(mimpos):
                            point = mimpos.towards(self.game_info.map_center, -1)
                            points.add(point)
                if len(points) == 0:
                    for (mimpos,mimt) in self.all_minerals:
                        point = mimpos.towards(self.game_info.map_center, -1)
    ...
    def set_rallypoint(self,goal):
        ...
        rally = goal.towards(self.game_info.map_center, 2)
        seen = False
                            ...
                            if toget > 0:
                                ...
                                for myscv in self.scvs_of_expo[expo]:
                                    if myscv.health >= 12:
                                        ...
                                        if job in (self.bad_jobs + self.no_jobs):
                                            recruits.add(myscv)
                                while (toget > 0) and (len(recruits) > 0):
                                    bestsd = 99999
                                    for myscv in recruits:
        ...
        for scv in self.units(SCV):
            ...
            if self.job_of_scvt[scvt] == 'fighter':
                ...
                if newphase == 'away':
                    if awaywalk:
                        newphase = 'walk'
                    elif self.sdist(enecenter, homemimpos) > self.sdist(mypos, homemimpos):
                        newphase = 'homemim'
                    else:
                        newphase = 'hismim'
                ...
                if do_action:
                    if newphase == 'hismim':
                        ...
                        scv.gather(homemim)
                        self.goal_of_unittag[scvt] = homemimpos
                    elif newphase == 'walk':
                        scv.move(awaypos)
            ...
            for wm in self.units(WIDOWMINE).ready:
                ...
                if self.near(wm.position,self.cheese1_landing_pos,7):
                    if wmt not in self.cheese1_mine_tags:
                        ...
                        found = False
                        while not found:
                            (mimpos, tag) = random.choice(tuple(self.all_minerals))  # mined out
    ...
    async def cheese3_internals(self):
        ...
        if self.cheese3_bunker2_pos != self.nowhere:
            if not self.we_started_at(BUNKER, self.cheese3_bunker2_pos):
                if self.allow_throw(BUNKER):
                    self.throw_at_spot(BUNKER, self.cheese3_bunker2_pos, 'cheese3_internals')
            ...
            if self.frame % 11 == 10:
                for (martype, bartype, pos, dura) in self.eggs:
                    if (dura >= 6) and (dura < 12):
                        ...
                        elif martype == STARPORTTECHLAB:
                            if (STARPORT,pos) in self.couldswap:
                                self.couldswap.remove((STARPORT, pos))
                ...
                for (thingtype,pos) in self.willswap:
                    for bu in self.structures(thingtype).ready.idle:
                        if bu.position == pos:
    ...
    async def ghost_fun(self):
        ...
        if self.ghost_phase == 'A':
            ...
            if self.we_started_amount(GHOSTACADEMY) < 1:
                if self.ghost_calmdown == 0:
                    if self.allow_throw(GHOSTACADEMY):
                        self.throw_anywhere(GHOSTACADEMY, 'ghost_fun')
            ...
            if self.we_started_amount(BARRACKS) < 2:
                if self.ghost_calmdown == 0:
                    if self.allow_throw(BARRACKS):
                        self.throw_anywhere(BARRACKS, 'ghost_fun')
                    self.ghost_calmdown = 80
            else:
                done += 1
            if self.we_started_amount(BARRACKSTECHLAB) < 1:
                if self.allow_throw(BARRACKSTECHLAB):
                    self.throw_anywhere(BARRACKSTECHLAB, 'ghost_fun')
                self.ghost_calmdown = 80
            else:
                done += 1
            if self.we_started_amount(FACTORY) < 1:
                if self.ghost_calmdown == 0:
                    if self.allow_throw(FACTORY):
                        self.throw_anywhere(FACTORY, 'ghost_fun')
                    self.ghost_calmdown = 80
            else:
                done += 1
            # continue
            ...
            if self.we_started_amount(GHOST) < 1:
                if self.ghost_calmdown == 0:
                    if self.allow_throw(GHOST):
                        self.throw_anywhere(GHOST, 'ghost_fun')
                    self.ghost_calmdown = 80
            else:
                done += 1
            if self.we_started_amount(PERSONALCLOAKING) < 1:
                if self.ghost_calmdown == 0:
                    if self.allow_throw(PERSONALCLOAKING):
                        self.throw_anywhere(PERSONALCLOAKING, 'ghost_fun')
                    self.ghost_calmdown = 80
            else:
                done += 1
                    ...
                    for scv in self.units(SCV):
                        ...
                        if job in self.bad_jobs + self.no_jobs:
                            if todo > 0:
                                ...
                                self.go_gather(scv,self.hisrampmim)
                        ...
                        if re_move:
                            if emotion == 'biting':
                                ...
                                if self.near(scv.position, self.hisrampmim[0], 2):
                                    self.go_gather(scv,self.homemim)
                                else:
                                    self.go_gather(scv,self.hisrampmim)
    ...
    def init_airports(self):
        for expo in range(0,self.expos):
            ...
            around = ccpos.towards(self.game_info.map_center, 5)
            ...
            around = ccpos.towards(self.game_info.map_center, -4)
...
# worker routines
    ...
    def be_mimminer(self,scv,mim):
        (mimpos,mimt) = mim
        if self.count_of_mimt[mimt] == 0: # first miner
            self.go_gather(scv,mim)
        self.mimt_of_minert[scv.tag] = mimt
        self.count_of_mimt[mimt] += 1
    ...
    def promote(self, scv,new_job):
        ...
        if old_job == 'gasminer':
            ...
            if scvt in self.mimt_of_minert:
                mimt = self.mimt_of_minert[scvt]
                self.count_of_mimt[mimt] -= 1
                del self.mimt_of_minert[scvt]
    ...
    async def manage_gas(self):
        ...
        if vacatures > 0:
            ...
            for scv in self.units(SCV):
                ...
                if (job == 'candidate'):
                    recruits.add(scv)
            if vacatures < len(recruits):
                for gas in self.all_refineries:
                    gast = gas.tag
                    if gast in self.all_gast:
                        todo = 3 - self.count_of_gast[gast]
                        for scv in recruits:
                            if self.job_of_scvt[scv.tag] == 'candidate': # scv not used
                                if todo > 0:
        ...
        if self.mimminer_vacatures()+self.gasminer_vacatures() > len(self.vision_of_scvt):
            ...
            for cc in self.all_bases:
                ...
                for gas in self.vespene_of_expo[expo]:
                    gast = gas.tag
                    if gast in self.all_gast:
                        self.wanted_of_cct[cct] += (3 - self.count_of_gast[gast])
            # decrease self.wanted_of_cct for walking applicants and candidates
    ...
    async def manage_minerals(self):
        ...
        for scvt in self.all_scvt:
            if self.job_of_scvt[scvt] == 'mimminer':
                if scvt not in self.mimt_of_minert:
                    for scv in self.units(SCV):
                        if scv.tag == scvt:
                            self.promote(scv,'idler')
        self.fix_count_of_job()
        ...
        if vacatures > 0:
            ...
            if vacatures < len(recruits):
                for mim in self.all_minerals:
                    (mimpos,mimt) = mim
                    if mimt in self.all_mimt:
                        todo = self.count_of_mimt[mimt] - 2
                        for scv in recruits:
                            if self.job_of_scvt[scv.tag] == 'candidate': # scv not assigned
                                if todo > 0:
                ...
                for scv in recruits:
                    scvt = scv.tag
                    freemimt = self.get_near_mimt_free(scv.position)
                    for mim in self.all_minerals:
                        (mimpos,mimt) = mim
                        if mimt == freemimt:
                            if self.near(scv.position,mimpos,self.miner_bound):
                                self.promote(scv,'mimminer')
                                self.be_mimminer(scv,mim)
        # after we tried local miner hiring, we want applicants
        if self.mimminer_vacatures()+self.gasminer_vacatures() > len(self.vision_of_scvt):
            self.wanted_of_cct = {}
            for cc in self.all_bases:
                cct = cc.tag
                ...
                for (mimpos,mimt) in self.minerals_of_expo[expo]:
                    if mimt in self.all_mimt:
                        self.wanted_of_cct[cct] += (2 - self.count_of_mimt[mimt])
            # decrease self.wanted_of_cct for walking applicants and candidates
            for scvt in self.vision_of_scvt:
                cct = self.vision_of_scvt[scvt]
                self.wanted_of_cct[cct] -= 1
    ...
    def speedmining(self):
        ...
        if self.supply_used >= 100:
            # normal mining
            # idle miners should mine
            for miner in self.units(SCV).idle:  # this is rare
                minert = miner.tag
                if minert in self.mimt_of_minert:
                    thismimt = self.mimt_of_minert[minert]
                    for mim in self.all_minerals:
        ...
        if self.supply_used < 100:
            ...
            for minert in self.mimt_of_minert:
            ...
            for miner in self.units(SCV):
                minert = miner.tag
                if minert in self.mimt_of_minert:
                    mimt = self.mimt_of_minert[minert]
                ...
                if minert in self.mimt_of_minert:
                    ...
                    if phase == 'G':
                        if self.near(miner.position, patchpoint, 3):
                            phase = 'I'
                            for othert in self.mimt_of_minert:
                                if othert != minert:
                                    othermimt = self.mimt_of_minert[othert]
                                    if mimt == othermimt:
                                        basepos = self.basepos_of_mimt[mimt]
                                        waitpoint = patchpoint.towards(basepos, 2)
            ...
            for scvt in self.all_scvt:
                if self.job_of_scvt[scvt] == 'idler':
                    if len(self.all_minerals) > 0:
                        ...
                        (mimpos,mimt) = mim
                        tries = 0
                        while (mimt in self.all_mimt) and (tries<100):
                            tries += 1
                            mim = random.choice(tuple(self.all_minerals))
                            (mimpos,mimt) = mim
                        for scv in self.units(SCV):
