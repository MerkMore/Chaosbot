...
class Chaosbot(sc2.BotAI):
    ...
    good_jobs = ('builder','settler','fencer','traveller','repairer','escorter','scout','cheeser','pilot')
    bad_jobs = ('gasminer','mimminer','applicant','defender','gasper','reporter')
    ...
    catchbarracks = 0 # count of barracks (at least half-built), minus count of marines etc in throwspots
    catchfactories = 0
    catchstarports = 0
    #
    ...
    async def my_init(self):
        ...
        # added constraints that do not exist
        self.init_techtree(ARMORY,FUSIONCORE) # sorry widowmines
        #
            ...
            if scv.tag not in self.speciality_of_tag:
                ...
                for mim in self.mineral_field:
                    if self.count_of_mimt[mim.tag] < 2:
                        sd = self.sdist(scv.position,mim.position) + 30 * self.count_of_mimt[mim.tag]
                        if sd < best_sd:
                            best_sd = sd
                            amim = mim
                            amimt = mim.tag
        ...
        # chat
        await self._client.chat_send('Chaosbot version 29 jun 2021, made by MerkMore', team_only=False)
        #
    ...
    def init_marine_opening(self, nbases):
        ...
        while bars < 4 * nbases:
            ...
            bars += 1
        # bunker placing
        # code deleted because of interaction with cocoon
        ...
        self.marine_gather_time = self.opening_create_units / 4
    ...
    def log_rich(self):
        if self.do_log_rich:
            print(' On '+str(self.frame)+' rich reservations:')
            for priority in [1,2,3]:
                for sta in ['dream','thought','prep']:
                    for (th,pl,st,ow, pri) in self.throwspots:
                        if (st == sta) and (pri == priority):
                            print('throwspot '+th.name+' '+self.txt(pl)+' '+st+' '+ow+' '+str(pri))
            for (item, goal, status) in self.buildorder_exe:
                print('buildorder '+item.name)
            if self.opening_create_units > 0:
                print('opening create units '+str(self.opening_create_units))
    ...
    def maxam_of_thing(self,thing) -> int:
        ...
        if thing in self.all_structures_tobuildwalk:
            ...
            if thing in [RAVEN]:
                maxam = 1
            if thing in [MARAUDER,SIEGETANK,WIDOWMINE,REAPER]:
                maxam = 15
            if thing == LIBERATOR:
                maxam = 5
        return maxam
    ...
    def rich(self,minerals,gas) -> bool:
        self.calculate_rich()
        return (self.vespene-self.purse.vespene >= self.rich_vespene + gas) and (self.minerals-self.purse.minerals >= self.rich_minerals + minerals)

    def rich_nogas(self,minerals) -> bool:
        # do i have minerals to throw a new thing?
        self.calculate_rich()
        return (self.minerals - self.purse.minerals >= self.rich_minerals + minerals)
        ...
        for base in self.all_mine_bases:
            ...
            if len(self.scvs_of_expo[expo]) >= 3:
                ...
                if sd < bestsd:
                    bestsd = sd
                    bestpos = pos
        # base.position.towards(self.game_info.map_center,5) better should be empty. See airport.
        return bestpos
    ...
    def get_near_mimt_free(self,point) -> int:
        ...
        # Stimulate taking close free ones.
        best_sdist = 99999
...
#*********************************************************************************************************************
    ...
    async def gamestate(self):
        ...
        for scv in self.units(SCV):
            if scv.tag in self.all_scvt:
                ...
                if scv.is_collecting:
                    if (job not in ['mimminer','gasminer','builder','volunteer','cheeser']):
        ...
        armybuildings += self.we_started_amount(FACTORY)
        armybuildings += self.we_started_amount(STARPORT)
        armybuildings += self.we_started_amount(GHOSTACADEMY)
        ...
        ccs += self.structures(ORBITALCOMMAND).amount
        #
        self.good_plan = 'UNKNOWN'
        best_satisfaction = 0
        if self.supply_used < 190:
            if self.we_started_amount(SCV) < 80: # 80 = max good_worker_supply
                satisfaction = (self.good_worker_supply+1) / (self.we_started_amount(SCV)+1)
                if satisfaction > best_satisfaction:
                    best_satisfaction = satisfaction
                    self.good_plan = 'workers'
            satisfaction = (self.good_army_supply+1) / (self.army_supply + 1)
            if satisfaction > best_satisfaction:
                best_satisfaction = satisfaction
                self.good_plan = 'army'
            satisfaction = (self.good_armybuildings+1) / (armybuildings+1)
            if satisfaction > best_satisfaction:
                best_satisfaction = satisfaction
                self.good_plan = 'armybuildings'
        satisfaction = (self.good_bases+1) / (ccs+1)
        if satisfaction > best_satisfaction:
            best_satisfaction = satisfaction
            self.good_plan = 'bases'
        if upgradebuildings < 4:
            satisfaction = (self.good_upgradebuildings+1) / (upgradebuildings+1)
            if satisfaction > best_satisfaction:
                best_satisfaction = satisfaction
                self.good_plan = 'upgradebuildings'
        if self.last_good_plan != self.good_plan:
            self.last_good_plan = self.good_plan
        ...
        #
        # count of barracks (at least half-built), minus count of marines etc in throwspots
        # used to cut ideas in throw_somewhere
        self.catchbarracks = 0
        for bar in self.structures(BARRACKS).ready:
            self.catchbarracks += 1
        for (marty,bartype,pos,dura) in self.eggs:
            if (marty == BARRACKS) and (dura < 20):
                self.catchbarracks += 1
        for (thing, pos, status, owner, priority) in self.throwspots:
            if self.cradle_of_thing(thing) == BARRACKS:
                self.catchbarracks -= 1
        self.catchfactories = 0
        for bar in self.structures(FACTORY).ready:
            self.catchfactories += 1
        for (marty,bartype,pos,dura) in self.eggs:
            if (marty == FACTORY) and (dura < 20):
                self.catchfactories += 1
        for (thing, pos, status, owner, priority) in self.throwspots:
            if self.cradle_of_thing(thing) == FACTORY:
                self.catchfactories -= 1
        self.catchstarports = 0
        for bar in self.structures(STARPORT).ready:
            self.catchstarports += 1
        for (marty,bartype,pos,dura) in self.eggs:
            if (marty == STARPORT) and (dura < 20):
                self.catchstarports += 1
        for (thing, pos, status, owner, priority) in self.throwspots:
            if self.cradle_of_thing(thing) == STARPORT:
                self.catchstarports -= 1
    ...
    def throw_at_spot(self, th, po, ow, pri):
        ...
        sta = 'dream'
        seen = False
        if self.cradle_of_thing(th) == BARRACKS:
            if self.catchbarracks <= 0:
                seen = True
                self.log_success('Thrown but not enough barracks for ' + th.name + ' by ' + ow)
        elif self.cradle_of_thing(th) == FACTORY:
            if self.catchfactories <= 0:
                seen = True
                self.log_success('Thrown but not enough factories for ' + th.name + ' by ' + ow)
        elif self.cradle_of_thing(th) == STARPORT:
            if self.catchstarports <= 0:
                seen = True
                self.log_success('Thrown but not enough starports for ' + th.name + ' by ' + ow)
        else:
            for (thing, pos, status, owner, priority) in self.throwspots:
            ...
            for scv in self.units(SCV):
                if scv.tag == scvt:
                    ...
                    if self.near(scv.position,goal,15):
                        ...
                        minsd = 99999
                        for mim in self.mineral_field:
                            sd = self.sdist(mim.position,goal)
                            if sd < minsd:
                                minsd = sd
                                bestmim = mim
                        if minsd == 99999:
...
#   army movement routines
    ...
    def marine_fun(self):
        if self.opening_name.find('marine') >= 0:
            ...
            # start the attack
            if (self.opening_create_units <= self.marine_gather_time) or (self.supply_used >= 180):
            ...
            if self.frame % 7 == 6:
                if self.frame < self.marine_attackframe + 22.4 * 60:
                    for mar in self.units(MARINE):
                        if mar.tag not in self.speciality_of_tag:
                            mar.attack(self.marine_goal)
                            self.speciality_of_tag[mar.tag] = self.opening_name
                ...
                if self.near(bc.position,dock,3):
                    ...
                    if len(self.scvs_of_expo[expo]) < 3:
                        ...
                        if betterdock != self.hisleft:
                            if betterdock != dock:
                                await self.go_jumpormove(bc, betterdock)
                ...
                if (mar.weapon_cooldown > 0):
                    ...
                    for miep in self.units_of_expo[expo]:
                        if miep.type_id == MARINE:
                            if self.near(miep.position,mar.position,1):
                                if miep.weapon_cooldown == 0:
                                    mieppos = miep.position
                    if mieppos != self.nowhere:
                        togo = mar.position.towards(mieppos, -0.5)
                        mar.move(togo)
        # marauders groupmove
        wantradius = 5
    ...
    def replatoon(self):
        ...
        for pid in range(0,len(self.platoons)):
            ...
            if plan == 'neutral':
                goal = self.enemy_pos.towards(self.game_info.map_center,-3)
            elif plan == 'agressive':
                goal = her
            elif plan == 'defensive':
                goal = Point2((3 * me.x - 2 * her.x, 3 * me.y - 2 * her.y))
            elif plan == 'flee':
                goal = self.loved_pos.towards(self.game_info.map_center,-3)
    ...
    def do_workerrush(self):
        if (self.opening_name.find('workerrush') >= 0) and (self.workerrushstate != 'ended'):
            ...
            # as the tags change, alas compute these each run.
            ...
            if self.sdist(self.hisleftmim.position, self.enemyramp_pos) < self.sdist(self.hisrightmim.position,
                                                                                     self.enemyramp_pos):
            ...
            for mim in self.minerals_of_expo[hisexpo]:
                if self.near(mim.position, self.enemy_pos, 8):
                    q = self.sdist(mim.position, self.hisleftmim.position) + self.sdist(mim.position,
                                                                                        self.hisrightmim.position)
                    if q < lowestq:
                        lowestq = q
                        self.hismidmim = mim
            # homemim
            ...
            for mim in self.mineral_field:
                ...
                if sd < bestsd:
                    ...
                    self.sidemim = mim
            # turningpoint
            self.turningpoint = self.enemy_pos.towards(self.hisrampmim.position,-3)
            #
            ...
            if self.workerrushstate == 'tobe':
                ...
                enemyarmy = 0
                for ene in self.enemy_units:
                    if ene.type_id not in {EGG, LARVA, OVERLORD, DRONE, SCV, PROBE, ZERGLING}:
                        enemyarmy += 1
                mustgoon = False
            ...
            for scv in self.units(SCV):
                if scv.tag in self.speciality_of_tag:
                    if self.speciality_of_tag[scv.tag] == 'workerrush':
                        ...
                        for order in scv.orders:
                            if order.ability.id == AbilityId.HARVEST_GATHER:
                                if order.target == self.hisrampmim.tag:
                                    orders = orders + '   jump hisrampmim'
                                elif order.target == self.hisfarmim.tag:
                                    orders = orders + '   jump hisfarmim'
                                elif order.target == self.hismidmim.tag:
                                    orders = orders + '   jump hismidmim'
                                elif order.target == self.sidemim.tag:
                                    orders = orders + '   jump sidemim'
                                elif order.target == self.homemim.tag:
                                    orders = orders + '   jump homemim'
                                else:
                                    orders = orders = '   jump ?'
                            elif type(order.target) == int:
                ...
                if scv.tag in self.speciality_of_tag:
                    if self.speciality_of_tag[scv.tag] == 'workerrush':
                        ...
                        if emotion == 'fleeing':
                            ...
                            if self.near(scv.position, self.hisfarmim.position, 8):
                                emotion = 'brave'
                            elif self.near(scv.position, self.hisrampmim.position, 8):
                                emotion = 'brave'
                        elif emotion == 'afraid':
                        ...
                        if emotion == 'ingoing':
                            ...
                            elif self.sdist(scv.position, self.hisfarmim.position) > self.sdist(scv.position,
                                                                                                self.hisrampmim.position):
                                tomim = self.hisrampmim
                            else:
    ...
    def airport(self, baseposition):
        # when building a cc, reserve airport place
        around = baseposition.towards(self.game_info.map_center, 5)
    ...
    def clean_layout(self):
        ...
        for (struc,place,tag) in self.designs:
            ...
            if place in self.tankplaces:
                seen = True
            if place in self.airportplaces:
                seen = True
