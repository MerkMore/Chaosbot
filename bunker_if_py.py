...
<<<<<<< HEAD
            if bunker_if.astate[mant] == 'resting':
                for man in bunker_if.units(SCV):
                    if man.tag == mant:
                        if bunker_if.near(man.position, bunker_if.hiding_spot, 6):
                            for ene in bunker_if.enemy_units:
                                if ene.type_id in {SCV,PROBE,DRONE}:
                                    if bunker_if.near(ene.position, bunker_if.hiding_spot, 12):
                                        if bunker_if.astate[mant] == 'resting':
                                            bunker_if.astate[mant] = 'disturbed'
                                            man.attack(ene)
            if bunker_if.astate[mant] == 'disturbed':
                for man in bunker_if.units(SCV):
                    if man.tag == mant:
                        if not bunker_if.near(man.position, bunker_if.hiding_spot, 12):
                            bunker_if.astate[mant] = 'returning'
